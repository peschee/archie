...
public class PatientsConsHistChart extends AbstractChartComposite {
	...
	protected AbstractDatasetCreator initializeCreator() {
		return new PatientsConsHistDatasetCreator(PatientsConsHistChart.CHART_TITLE);
	}
	...
	public void refresh() {
		...
		final IPreferenceStore preferences = ArchieActivator.getDefault().getPreferenceStore();
		if(preferences.getInt(PreferenceConstants.P_COHORT_SIZE) > 0) {
			((PatientsConsHistDatasetCreator) this.creator).setCohortSize(preferences.getInt(PreferenceConstants.P_COHORT_SIZE));
		}
	}
}
