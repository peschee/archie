...
public class StatisticsView extends ViewPart implements UserListener {
	...
	private void initialize() {
		...
		if (ArchieACL.userHasAccess()) {
			this.message = new GraphicalMessage(this.parent, ArchieActivator.getImage(ArchieActivator.IMG_WARNING),
					Messages.NO_PLUGIN_SELECTED);
		} else {
			this.message = new GraphicalMessage(this.parent, ArchieActivator.getImage(ArchieActivator.IMG_ERROR),
	 ...
	 */
	public void clean() {
		for(Control child : this.parent.getChildren()) {
			child.dispose();
		}
	}
